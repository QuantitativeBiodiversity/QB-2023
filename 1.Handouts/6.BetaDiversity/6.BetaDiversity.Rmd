---
title: '6\. Among site (beta) Diversity'
...
## OVERVIEW
In this handout, we move beyond the investigation of within-site $\alpha$-diversity.
...
After working through this handout, you will know how to:
...
rm(list = ls())
getwd()
setwd("~/GitHub/QB-2023/1.HandOuts/6.BetaDiversity")
```
...
Let us load `doubs` data set, which is in the `ade` package.
...
Let us use `vegdist` to create a resemblance matrix for the fish assemblages in the Doubs River. 
...
Now that we have created a resemblance matrix, it would be nice to visualize the fish assemblages of the Doubs River. 
