...
Hill number are becoming popular in biodiversity research for a number of reasons. First, they are all expressed in common units: the effective number of species. Second, it is easy to convert other commonly used measured of diversity into Hill numbers. Third, Hill numbers address the **replication principle** also known as the **doubling property**. Consider two completely distinct assemblages where there are no species in common, but there are identical relative abundance distributions. In this case, the Hill number will double when the samples are combined. Fourth, Hill numbers an accommodate taxonomic, phylogenetic, and functional information.

Here, we will use `iNEXT` package to calculate Hill numbers as a function of sample coverage using `site 1` and `site 2` of the BCI site-by-species matrix.
...
Now let us graphically explore the Hill numbers. 
We will bootstrap the estimates with random sampling to determine the confidence intervals around diversity estimations. 
