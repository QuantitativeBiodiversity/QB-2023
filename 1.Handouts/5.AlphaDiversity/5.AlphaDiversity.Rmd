...
Diversity indices we discussed above combine species richness and the proportion of each species into a single metric. However, these indices tend to be highly correlated with one another, and are not always expressed in units that allow comparisons. Jost (2002) argued that to call Shannon entropy and Simpson index as diversity is misleading, since they are not in units of species (Shannon are in units of bits and Simpson is probability). **Effective number of species** concept (MacArthur 1965) overcame this this problem. It refers to the number of equally abundant species needed to obtain the same mean proportional species abundance as that observed in the dataset of interest (where all species may not be equally abundant). Each of the indices above can be converted into effective number with a simple formula. Hill (1973) realized that species richness, Shannon entropy and Simpson index are all members of the same family of diversity indices, later called as Hill numbers. Hill numbers are mathematically unified differing among themselves only by an exponent $q$. Exponent $q$ quantifies how much the measure discounts rare species when calculating diversity. Hill numbers are reintroduced to ecologists by Jost (2006, 2007) after McArthur and Hill. Advantages of them over other diversity indices can be summarized as below: 

1. Hill numbers are all expressed in units of effective numbers of species. 
3. Key diversity indices proposed in the literature, including Shannon entropy and the Gini-Simpson index, can be easily converted to Hill numbers (see Table below). 
2. Hill numbers do not violate mathematical **replication principle** or **doubling property**. Consider two completely distinct assemblages (i.e., no species in common). Doubling property implies that if
they have identical relative abundance distributions, then the Hill number doubles when the assemblages are combined with equal weights. 
4. Fourth, Hill numbers can be effectively generalized to incorporate taxonomic, phylogenetic, and functional diversity, and thus provide a unified framework. 
5. When multiple assemblages are compared, there is a direct link between Hill numbers and compositional similarity uniting the diversity and similarity (or differentiation). 

The generalized Hill equation is:

$$\displaystyle ^qD=\biggl(\sum_{i=1}^{S}p_i^q\biggl)^\frac{1}{1-q}$$
Where the $q$ exponent is the order in which the index is dependent on rare species. For $q = 0$, 1 and 2 (also noted as $N_0$, $N_1$ and $N_2$): $^0D=S$ (species richness), $^1D=e^H$ (exponential Shannon entropy),  $^2D=1/D$ (reciprocal of Simpson index).

Here we will use **iNEXT** package to graphically explore Hill diversity incorporating sample coverage. It will allow us to extrapolate diversity beyond the samples available. Let us do this comparison using the `site 1` and `site 2` of the BCI site-by-species matrix.

Let us load the package and prepare the data: 


```{r}
#Uncomment the following line if iNEXT package is not installed on your computer
...
Now let us graphically explore the Hill numbers. We will bootstrap the estimates with random sampling to determine the confidence intervals around diversity estimations. 
