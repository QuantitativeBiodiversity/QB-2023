...
It is not possible to cover all of these topics in Quantitative Biodiversity, but we try to highlight some of the major concepts in this handout, including the following table:
...
A downside of ML estimation is that it is a computationally intensive process. This is because ML estimation needs to find the tree that gives the data the highest probability, a difficult task that requires the use of optimized algorithms. 
...
# Bootstrapping is computationally expensive, so it will take a couple minutes to run.
bs <- bootstrap.pml(fitGTR, bs = 100, optNni = TRUE, control = pml.control(trace = 0))

# Plot your bootstrap values onto the tree to visualize results. We will plot bootstrap replicates greater than 50%
...
With this information we will visualize the functional traits (i.e., growth on different forms of phosphorus) in a phylogenetic context by mapping them onto the neighbor joining tree constructed under the F84 DNA substitution model (see the table on substitution models) that we created. 
...
Now, let us generate quantitative output that will allow us to assess how much phylogenetic signal is in our data set.
...
So far we have quantified the distribution of traits along the phylogeny. We can also draw a dendrogram based on our traits to visualize similarities in traits among species independent of phylogeny. This will allow us to visualize the degree that, for example, phylogeny, resource use, and environment influence the species we are studying. For the case of our phosphorus use data, we can see the degree that phylogenetic relationships and lake origin have on our resource use data. To create a dendrogram of resource use, we will make a hierarchical agglomerative cluster of the binary trait data. The  benefit is that we get an additional layer of information from clustering resource use. A hierarchical cluster will allow us to visualize the pairwise similarity among species for resource use, which we can use to infer **niche overlap**. Niche overlap indicates the potential for exploitative competition among taxa at each site because two taxa may compete for the same resource. Here we will map our trait data onto the phylogeny to compare the hierarchical structures in each dataset. We can compare our two dendrograms using a **tanglegram** (a plot that shows the alignment between two dendrograms), Robinson-Fould metric, and a Mantel correlation test.
```{r}
...
# Mantel test to correlate patristic distance (pairwise sum of branch lengths) in phylogeny and Jaccard index in the resource use hierarchical cluster.
